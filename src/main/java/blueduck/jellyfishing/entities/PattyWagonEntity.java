...
public class PattyWagonEntity extends Entity {
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(PattyWagonEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(PattyWagonEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(PattyWagonEntity.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(PattyWagonEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> LEFT_PADDLE = EntityDataManager.createKey(PattyWagonEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> RIGHT_PADDLE = EntityDataManager.createKey(PattyWagonEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(PattyWagonEntity.class, DataSerializers.VARINT);
    ...
    private PattyWagonEntity.Status status;
    private PattyWagonEntity.Status previousStatus;
    ...
    public PattyWagonEntity(World worldIn, double x, double y, double z) {
        this(JellyfishingEntities.PATTY_WAGON.get(), worldIn);
    ...
    protected void registerData() {
        ...
        this.dataManager.register(BOAT_TYPE, PattyWagonEntity.Type.OAK.ordinal());
...
//    }
    ...
    public Item getItemBoat() {
        return JellyfishingItems.AIR_SUIT_BOOTS.get();
    }
    ...
    public void tick() {
        ...
        if (this.status != PattyWagonEntity.Status.UNDER_WATER && this.status != PattyWagonEntity.Status.UNDER_FLOWING_WATER) {
     ...
     */
    private PattyWagonEntity.Status getBoatStatus() {
        PattyWagonEntity.Status boatentity$status = this.getUnderwaterStatus();
        if (boatentity$status != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return boatentity$status;
        } else if (this.checkInWater()) {
            return PattyWagonEntity.Status.IN_WATER;
        } else {
            float f = this.getBoatGlide();
            if (f > 0.0F) {
                this.boatGlide = f;
                return PattyWagonEntity.Status.ON_LAND;
            } else {
                return PattyWagonEntity.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private PattyWagonEntity.Status getUnderwaterStatus() {
        AxisAlignedBB axisalignedbb = this.getBoundingBox();
                ...
                for(int i2 = i1; i2 < j1; ++i2) {
                    ...
                    if (fluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$mutable.getY() + fluidstate.getActualHeight(this.world, blockpos$mutable))) {
                        if (!fluidstate.isSource()) {
                            return PattyWagonEntity.Status.UNDER_FLOWING_WATER;
                        }
        ...
        return flag ? PattyWagonEntity.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion() {
        ...
        double d2 = 0.0D;
        this.momentum = 0.05F;
        if (this.previousStatus == PattyWagonEntity.Status.IN_AIR && this.status != PattyWagonEntity.Status.IN_AIR && this.status != PattyWagonEntity.Status.ON_LAND) {
            ...
            this.lastYd = 0.0D;
            this.status = PattyWagonEntity.Status.IN_WATER;
        } else {
            if (this.status == PattyWagonEntity.Status.IN_WATER) {
                d2 = (this.waterLevel - this.getPosY()) / (double)this.getHeight();
                this.momentum = 0.9F;
            } else if (this.status == PattyWagonEntity.Status.UNDER_FLOWING_WATER) {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            } else if (this.status == PattyWagonEntity.Status.UNDER_WATER) {
                d2 = (double)0.01F;
                this.momentum = 0.45F;
            } else if (this.status == PattyWagonEntity.Status.IN_AIR) {
                this.momentum = 0.9F;
            } else if (this.status == PattyWagonEntity.Status.ON_LAND) {
...
//    }
    ...
    protected void readAdditional(CompoundNBT compound) {
        if (compound.contains("Type", 8)) {
            this.setBoatType(PattyWagonEntity.Type.getTypeFromString(compound.getString("Type")));
        }
        ...
        if (!this.isPassenger()) {
            if (onGroundIn) {
                if (this.fallDistance > 3.0F) {
                    if (this.status != PattyWagonEntity.Status.ON_LAND) {
    ...
    public void setBoatType(PattyWagonEntity.Type boatType) {
        this.dataManager.set(BOAT_TYPE, boatType.ordinal());
    }

    public PattyWagonEntity.Type getBoatType() {
        return PattyWagonEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
    }
    ...
    public boolean canSwim() {
        return this.status == PattyWagonEntity.Status.UNDER_WATER || this.status == PattyWagonEntity.Status.UNDER_FLOWING_WATER;
    }
    ...
    public static enum Type {
         ...
         */
        public static PattyWagonEntity.Type byId(int id) {
            PattyWagonEntity.Type[] aboatentity$type = values();
        ...
        public static PattyWagonEntity.Type getTypeFromString(String nameIn) {
            PattyWagonEntity.Type[] aboatentity$type = values();
